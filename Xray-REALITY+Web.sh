...
openssl_version="openssl-openssl-3.6.0"
...
get_cert()
{
    ...
    if ! "$HOME/.acme.sh/acme.sh" --installcert -d "${true_domain_list[$1]}" --key-file "${nginx_prefix}/certs/${true_domain_list[$1]}.key" --fullchain-file "${nginx_prefix}/certs/${true_domain_list[$1]}.cer" --reloadcmd "systemctl force-reload nginx && sleep 2s" --ecc; then
